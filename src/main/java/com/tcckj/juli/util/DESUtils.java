...
public class DESUtils {
    private static final String FILE_NAME = "a.a";
    /**
     * 保存账号密码
     * @param context
     * @param number
    ...
    public static void savePassword(Context context, String number, String password, boolean isRemember) {
        //对数据进行加密
        //得到key
        SecretKey key = DESUtils.readKey(DESUtils.getPath(FILE_NAME));
        if (key == null) {
            key = get3DESKey();
            //保存key
            DESUtils.saveKey(key, DESUtils.getPath(FILE_NAME));
        }
    ...
    public static String d(String str) {
        if (!TextUtils.isEmpty(str)) {
            //对数据进行解密
            SecretKey key = readKey(DESUtils.getPath(FILE_NAME));
            if (key != null) {
